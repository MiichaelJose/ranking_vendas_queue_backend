...
import {
    ...
    Status4XXNames,
    Status5XXNames,
} from '@shared/helpers/http-status-schema'
...
function appErrorMiddleware({
    ...
    return (
        ...
        const defaultStatusErrorInternal =
            typeof defaultStatusError === 'number'
                ? defaultStatusError
                : selectStatus(defaultStatusError)

        return response
            .status(error.status || defaultStatusErrorInternal || 500)
            .json({
                errors: [
                    {
