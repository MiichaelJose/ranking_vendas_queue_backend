...
class CreateOrderQueueController {
    async handle(request: Request, response: Response) {
        ...
        const dataNormalized = await gatewayIntegrations[gateway].normalizeData(
            projectId,
            request.body as ICreateOrderDTO
        )

        const createOrderQueueUseCase = container.resolve(
            CreateOrderQueueUseCase
        )

        const data = await createOrderQueueUseCase.execute(
            projectId,
            dataNormalized
        )
