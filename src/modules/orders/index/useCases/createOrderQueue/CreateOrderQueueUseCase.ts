...
class CreateOrderQueueUseCase {
    ...
    async execute(projectId: string, data:  ICreateOrderDTO): Promise<ICreateOrderDTO> {

        const inactiveProject: boolean = await this.orderRepository.getStatusProjectByProjectId(projectId)
        ...
        const participantsTeamProject = await this.orderRepository.getProjectUsers(projectId)
        ...
        orders.forEach(order => {
            ...
            if (userId !== undefined && participantUsageCount[userId] !== undefined) {
        ...
        const nextParticipant = participantsTeamProject.reduce((prev, current) => {
